...
# fft2melmx.py
#
# based on https://github.com/jorgehatccrma/pyagc/blob/master/agc/utils.py
#
...
def fft2melmx(nfft, sr=8000., nfilts=None, width=1., minfrq=0., maxfrq=None, htkmel=False, amplitude='ortho'):
    """
    ...
    amplitude = 'ortho' (default) - normalize loop gain fft->mel->fft
                'norm'  - normalize the sum of each row
                'const' - largest value in each row is 1
    ...
    if amplitude == 'norm':
        ...
        wts *= (2. / (binfrqs[2:nfilts + 2] - binfrqs[:nfilts]))[:, np.newaxis]
    elif amplitude == 'ortho':
        #wts /= np.sum(wts**2, axis=1)[:, np.newaxis]
        wts *= np.sqrt((2. / (binfrqs[2:nfilts + 2] - binfrqs[:nfilts]))[:, np.newaxis])
    elif amplitude == 'const':
        pass
    else:
        ValueError('unrecognized value for amplitude')
...
def mel2hz(z, htk=False):
    ...
    if htk:
        f = 700. * (10. ** (z / 2595.) - 1)
        # f = 700. * (np.exp(z / 1127.0) - 1.0) # wpd, 20170912, Kaldi version of mel scale
    else:
        f_0 = 0  # 133.33333
        f_sp = 200. / 3.  # 66.66667
        brkfrq = 1000.
...
def hz2mel(f, htk=False):
    ...
    if htk:
        ...
        # z = 1127.0 * np.log(1.0 + f / 700.0) # wpd, 20170912, Kaldi version of mel scale
    else:
        # pass
        f_0 = 0  # 133.33333;
        f_sp = 200. / 3.  # 66.66667;
